...
class BackwoardOH extends React.Component {
    constructor() {
        super();
        this.state = {
            fx: "",
        ...
        this.BackwoardOH = this.BackwoardOH.bind(this);
    }

    componentDidMount() {
        axios.get("http://192.168.99.100:9000/api/getbwoh").then(res => {
    ...
    BackwoardOH(d, h, x) {
        ...
        if (this.state.pow === 1) {
            if (d === 1) {
                ans = (this.func(x) - this.func(x - h)) / h
                real = this.funcd(x)
            } else if (d === 2) {
                ans = (this.func(x) - 2 * this.func(x - h) + this.func(x - (2 * h))) / Math.pow(h, 2)
                real = this.funcd2(x)
            } else if (d === 3) {
                ans = (this.func(x) - 3 * this.func(x - h) + 3 * this.func(x - (2 * h)) - this.func(x - (3 * h))) / Math.pow(h, 3)
                real = this.funcd3(x)
            } else if (d === 4) {
                ans = (this.func(x) - 4 * this.func(x - h) + 6 * this.func(x - (2 * h)) - 4 * this.func(x - (3 * h)) + this.func(x - (4 * h))) / Math.pow(h, 4)
                real = this.funcd4(x)
            }
        }else{
            if (d === 1) {
                ans = (3 * this.func(x) - 4 * this.func(x - h) + this.func(x - (2 * h))) / (2 * h)
                real = this.funcd(x)
            } else if (d === 2) {
                ans = (2 * this.func(x) - 5 * this.func(x - h) + 4 * this.func(x - (2 * h)) - this.func(x - (3 * h))) / Math.pow(h, 2)
                real = this.funcd2(x)
            } else if (d === 3) {
                ans = (5 * this.func(x) - 18 * this.func(x - h) + 24 * this.func(x - (2 * h)) - 14 * this.func(x - (3 * h)) + 3 * this.func(x - (4 * h))) / (2 * (Math.pow(h, 3)))
                real = this.funcd3(x)
            } else if (d === 4) {
                ans = (3 * this.func(x) - 14 * this.func(x - h) + 26 * this.func(x - (2 * h)) - 24 * this.func(x - (3 * h)) + 11 * this.func(x - (4 * h)) - 2 * this.func(x - (5 * h))) / (Math.pow(h, 4))
                real = this.funcd4(x)
            }
        }
        console.log("cal :", ans)
        ...
        return (
            <Layout>
                <Breadcrumb style={{ margin: '16px 0' }}>
                    ...
                    <Breadcrumb.Item>Backward OH</Breadcrumb.Item>
                </Breadcrumb>
                <div onChange={this.handleChange}>
                    ...
                    <Button onClick={() => this.BackwoardOH(parseFloat(this.state.d), parseFloat(this.state.h), parseFloat(this.state.x))} size="large" type="primary">SUMMIT  </Button>
                </div><br></br>
